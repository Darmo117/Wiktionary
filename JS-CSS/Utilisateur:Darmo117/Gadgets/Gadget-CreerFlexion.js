 ...
 * ------------------------------------------------------------------
 * [[Catégorie:JavaScript du Wiktionnaire|CreerFlexion-dev.js]]
 */
$(function () {
  window.wikt.gadgets.creerFlexion = {
    baseWord: "",

    inflectionToCreate: "",

    /**
     * @type {Array<wikt.gadgets.creerFlexion.LanguageData>}
     */
    _languages: [],

    /**
     * Adds the given language data to this gadget.
     * @param languageData {wikt.gadgets.creerFlexion.LanguageData}
     */
    ...
    init: function () {
      this._languages.forEach(function (languageData) {
        ...
        $(flexTableClass + " .new").each(function () {
          var $redLink = $(this);
          ...
          // redLinkGender est undefined dans le cas de fr-rég, où le genre n'est pas
          ...
          $redLink.click(function (e) {
            e.preventDefault();
            this.baseWord = mw.config.get("wgPageName").replace(/_/g, " ");
            this.inflectionToCreate = inflection.replace(/&nbsp;/g, " ");
            this._onClick();
          });
        });
      });
     ...
     * @param response {Object} The response containing the wikicode for the current page.
     * @private
     */
    _onSuccess: function (response) {
      var wikicode = response.responseText;
      var wikicodeLines = wikicode.split("\n");

      // Avorter la création si la page de base ne contient pas le wikicode nécessaire
      if (!/== *{{langue\|fr}} *==/.test(wikicode)) {
        alert("Pas de section Français reconnue\u00a0!\nCréation avortée");
        return;
      }
      if (!/=== *{{S\|(adj|adjectif|nom)\|fr[|}]/.test(wikicode)) {
        ...
        switch (status) {
          ...
          case "wait frm adj" :
            ...
            if (lig_frm) {
              ...
              if (/fr-accord-al\|/.test(newWikicode))
                ...
                newWikicode = Flex_reg2(newWikicode, CrFlFr_Mot);
              else
                newWikicode += Flex_standard(wikicodeLines[k], CrFlFr_Mot);
          ...
          case "wait frm nom" :
            ...
            if (lig_frm) {
              ...
              var lfm = wikicodeLines[k].replace(CrFlFr_Mot, CrFlFr_Flex);
              ...
              newWikicode += CrFlFr_Mot + "#fr|" + CrFlFr_Mot + "]].\n\n";
              status = "wait sec mot";
            }
            break;
        }
      ...
      //générer clé de tri
      var sortingKey = wikt.page.getSortingKey(CrFlFr_Flex);
      if (sortingKey !== CrFlFr_Flex) {
   ...
   * @param genderClasses {Array<string>}
   * @constructor
   */
  wikt.gadgets.creerFlexion.LanguageData = function (languageCode, inflectionTableClass, genderClasses) {
    this._languageCode = languageCode;
    this._inflectionTableClass = inflectionTableClass;
    this._genderClasses = genderClasses;
  };

  wikt.gadgets.creerFlexion.LanguageData.prototype = {
  };

  Object.defineProperty(wikt.gadgets.creerFlexion.LanguageData.prototype, "languageCode", {
    /**
  ...
  Object.defineProperty(wikt.gadgets.creerFlexion.LanguageData.prototype, "inflectionTableClass", {
    /**
  ...
  Object.defineProperty(wikt.gadgets.creerFlexion.LanguageData.prototype, "genderClasses", {
    /**
     * @return {Array<string>} The list of CSS classes for each gender.
     */
    get: function () {
  ...
  var CrFlFr_Mot = ""; // TODO supp
  var CrFlFr_Flex = ""; // TODO supp
  ...
  //--------------------------------------------------------------------------------------
  function CrFlFr_CreerFlexion3(request, data) {
    while (document.body.firstChild) {
      document.body.removeChild(document.body.firstChild);
    }
    document.body.innerHTML = request.responseText;
    document.getElementById("wpTextbox1").value = data.text;
    document.getElementById("wpSummary").value = "Création avec [[Aide:Gadget-CreerFlexion|Gadget-CreerFlexion]]" +
...
//--------------------------------------------------------------------------------------
  ...
  // Only enabled in main namespace.
  if (wikt.page.getCurrentPageNamespaceId() === mw.config.get("wgNamespaceIds")[""]) {
    wikt.gadgets.creerFlexion.init();
  }
});
