/**
 * This gadget lets users create inflection pages for french
 * words from red links inside inflection tables.
 *
 * Gadget original : ArséniureDeGallium 2012-11-07
 * Réécriture : Darmo117 2020-06-?? TODO màj date à la sortie
 *
 * [[Catégorie:JavaScript du Wiktionnaire|CreerFlexionFr.js]]
 */
window.wikt.gadgets.creerFlexionFr = {
  baseWord: "",
  inflectionToCreate: "",
  ...
  init: function () {
    var genre_pagename;
    var $flextableSelfLink = $(".flextable-fr-mfsp .selflink");
    var flextableExists = $flextableSelfLink.length;

    if (flextableExists) {
      if ($flextableSelfLink.first().parents(".flextable-fr-m").length) {
        genre_pagename = "m";
      }
      else if ($flextableSelfLink.first().parents(".flextable-fr-f").length) {
        genre_pagename = "f";
      }
    }
    ...
    $(".flextable-fr-mfsp .new").each(function () {
      ...
      var isNoun = false;
      var wordType = $redLink.parents(".flextable-fr-mfsp")
      ...
      var redLinkGender;
      if ($redLink.parents(".flextable-fr-m").length) {
        redLinkGender = "m";
      }
      else if ($redLink.parents(".flextable-fr-f").length) {
        redLinkGender = "f";
      }
      ...
      if (isNoun && (redLinkGender !== genre_pagename && redLinkGender)) {
        return;
      }

      $redLink.css("background-color", "#00ff00");
      $redLink.prop("title", "Cliquez pour créer « {0} » avec le gadget CréerFlexionFr".format(inflection));
   ...
   * @private
   */
  _performStep2: function (response) {
...
if (wikt.page.getCurrentPageNamespaceId() === mw.config.get("wgNamespaceIds")[""]) {
  wikt.gadgets.creerFlexionFr.init();
}
