 ...
 * v5.0.8 2020-08-25 added {{type}} template to verbs, removed lang parameter for some
 *                   interwiki templates, gadget now collapses when code is inserted
 * ------------------------------------------------------------------------------------
...
mw.loader.using(["oojs-ui-core", "oojs-ui-widgets", "oojs-ui-toolbars", "oojs-ui-windows"], function () {
  ...
  wikt.gadgets.creerNouveauMot.StartGui = function (gadgetName, onActivateGadget) {
    ...
    var headerWikicode = (
        "{{début encart coloré}}\n" +
        "{{information}} '''''{0}''''' est un outil " +
        "qui vous aide à ajouter des mots sur le Wiktionnaire " +
        "sans avoir besoin de tout comprendre à la syntaxe wiki. " +
        "Voir [[Aide:Gadget-CreerNouveauMot|l’aide]] pour plus d’explications.\n" +
        "{{fin encart}}"
    ).format(gadgetName);
    headerText += wikt.page.renderWikicode(headerWikicode).html();
